...
class EMNISTDataProvider(DataProvider):
    ...
    def label_smoothing(self, int_targets, alpha=0.1):
        ...
        """
        # Initialise a matrix filled with alpha / (self.num_classes - 1)
        one_of_k_targets = np.full((int_targets.shape[0], self.num_classes), alpha / (self.num_classes - 1))
        # If correct classes, set to 1 - alpha
        one_of_k_targets[range(int_targets.shape[0]), int_targets] = 1 - alpha
